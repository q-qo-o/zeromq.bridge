...
class ZmqPublishIntArrayInternalState:
    def __init__(self):
        self.mgr = ZmqManager.get_instance()
        self.logged_pub = False

class ZmqPublishIntArray:
    @staticmethod
    def internal_state():
        return ZmqPublishIntArrayInternalState()
    ...
    def compute(db) -> bool:
        ...
        try:
            ...
            if not state.logged_pub:
                carb.log_info(f"ZMQ int array publishing started: addr={address}, topic={topic}")
                state.logged_pub = True

            db.outputs.execOut = db.inputs.execIn
            
        except Exception as e:
            carb.log_error(f"Failed to publish ZMQ int array: {e}")
            return False
