# zeromq/bridge/ogn/python/nodes/ZmqSubscribeDouble.py
from zeromq.bridge.impl.zmq_manager import ZmqManager
import carb

class ZmqSubscribeDoubleInternalState:
    def __init__(self):
...
class ZmqSubscribeDouble:
    @staticmethod
    def internal_state():
        return ZmqSubscribeDoubleInternalState()
    ...
    def compute(db) -> bool:
        ...
        try:
            ...
            if topic != state.last_topic or not state.subscription_requested:
                state.mgr.request_subscription(topic, "Float64")
            ...
            db.log_error(f"Failed to receive ZMQ double: {e}")
            return False
