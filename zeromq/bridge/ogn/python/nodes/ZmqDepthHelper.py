...
class ZmqDepthHelperInternalState:
    def __init__(self):
...
class ZmqDepthHelper:
    @staticmethod
    def internal_state():
        return ZmqDepthHelperInternalState()
    ...
    def compute(db) -> bool:
        ...
        try:
            ...
            if state.target_path != target_path or not state.target_prim or not state.target_prim.IsValid():
                ...
                if not prim or not prim.IsValid():
                    if state.target_path != target_path:
                        db.log_warning(f"ZmqDepthHelper: Target prim {target_path} not found")
                    state.clear()
                    db.outputs.execOut = db.inputs.execIn
                    return True
            ...
            # Calculate Depth: water_surface - current_z
            # If current_z > water_surface (above water), depth < 0.
            # Requirement: If above surface (depth < 0), output 0.
            depth = water_surface - current_z
            if depth < 0:
                depth = 0.0
            ...
            if not state.logged_pub:
                db.log_info(f"ZMQ Depth data published: topic={topic}, addr={address}")
                state.logged_pub = True
                
            db.outputs.execOut = db.inputs.execIn

        except Exception as e:
            db.log_error(f"Failed to publish ZMQ Depth data: {e}\n{traceback.format_exc()}")
            return False
