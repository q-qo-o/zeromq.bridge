...
class ZmqSubscribeFloatArrayInternalState:
    def __init__(self):
...
class ZmqSubscribeFloatArray:
    @staticmethod
    def internal_state():
        return ZmqSubscribeFloatArrayInternalState()
    ...
    def compute(db) -> bool:
        ...
        try:
            ...
            if topic != state.last_topic or not state.subscription_requested:
                state.mgr.request_subscription(topic, "Float32MultiArray")
            ...
            carb.log_error(f"Failed to receive ZMQ float array: {e}")
            return False
