...
class ZmqSubscribeIntArrayInternalState:
    def __init__(self):
...
class ZmqSubscribeIntArray:
    @staticmethod
    def internal_state():
        return ZmqSubscribeIntArrayInternalState()
    ...
    def compute(db) -> bool:
        ...
        try:
            ...
            if topic != state.last_topic or not state.subscription_requested:
                state.mgr.request_subscription(topic, "Int32MultiArray")
            ...
            carb.log_error(f"Failed to receive ZMQ int array: {e}")
            return False
